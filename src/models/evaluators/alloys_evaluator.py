...
try:
    ...
    print(f"[AlloysEvaluator] Warning: Could not import 'shap'. SHAP analysis will be unavailable. Error: {e}")
    shap = None
...
class AlloysEvaluator(BaseEvaluator):
    ...
    def _predict_and_unscale(self, model: Any, data: Optional[Dict[str, np.ndarray]], data_is_scaled: bool = True) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        """
        Makes predictions on the given data and inverse transforms the output.
        
        Args:
            model: The trained model
            data: Dictionary containing 'X' and 'y' data
            data_is_scaled: Whether the data['y'] is already scaled (True by default in pipeline)
        """
        if data is None:
            return None, None
        ...
        # Apply inverse transform if data is scaled and scaler is available
        if self.target_scaler is not None and data_is_scaled:
            y_true_unscaled = self._inverse_transform_y(y_true)
        else:
            # Data is already unscaled or no scaler available
    ...
    def evaluate_model(self,
        ...
        # Note: In the training pipeline, all data (train, val, test) is scaled
        train_y_unscaled, train_pred_unscaled = self._predict_and_unscale(model, train_data, data_is_scaled=True)
        val_y_unscaled, val_pred_unscaled = self._predict_and_unscale(model, val_data, data_is_scaled=True)
        test_y_unscaled, test_pred_unscaled = self._predict_and_unscale(model, test_data, data_is_scaled=True)
